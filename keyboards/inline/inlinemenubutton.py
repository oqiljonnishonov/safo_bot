...
uzbek= InlineKeyboardButton(text="O'zbek", callback_data=fan_callback.new(item_name='uzbek'))
change_language.insert(uzbek)
english=InlineKeyboardButton(text="English", callback_data=fan_callback.new(item_name='english'))
change_language.insert(english)
russian=InlineKeyboardButton(text="Russian", callback_data=fan_callback.new(item_name='russian'))
...
menu_cd = CallbackData("show_menu", "level", "category", "subcategory", "item_id", "number", "summ", "minus")
...
def make_callback_data(level, category="0", subcategory="0", item_id="0", number="0", summ="0", minus="0"):
    return menu_cd.new(
        level=level, category=category, subcategory=subcategory, item_id=item_id, number=number, summ=summ, minus=minus
    )
...
async def categories_keyboard():
    ...
    markup.row(
        InlineKeyboardButton(
            text=f"🛒 Savatni ko'rish", callback_data="savat"
        )
    )
...
def buy_item_data(level, count="0", item_id="0"):
    print('inline 107')
    return buy_item.new(
        level=level, count=count, item_id=item_id
    )
#######
...
def item_keyboard(category, subcategory, item_id, number, summ, minus):
    ...
    if int(number) > 0 or int(summ) > 0:
        ...
        for item in range(1, 10):
            ...
            temp.append(InlineKeyboardButton(text=button_text, callback_data=callback_data))
            if len(temp) > 2:
                markup.row(*temp)
                temp.clear()
        ...
        markup.row(
            InlineKeyboardButton(
                text=f"0", callback_data=make_callback_data(
                    level=CURRENT_LEVEL,
                    category=category,
                    subcategory=subcategory,
                    item_id=item_id,
            ...
            InlineKeyboardButton(
                text=f"◀", callback_data=make_callback_data(
                    level=CURRENT_LEVEL + 1,
                    category=category,
                    subcategory=subcategory,
                    item_id=item_id,
        ...
        markup.row(
            InlineKeyboardButton(
                text=f"➕ Savatga qo'shish", callback_data=buy_item_data(
                    level=0,
                    count=r_number,
                    item_id=item_id,
        ...
        markup.row(
            InlineKeyboardButton(
                text="⬅️Ortga",
                callback_data=make_callback_data(
                    level=CURRENT_LEVEL - 1, category=category, subcategory=subcategory, item_id=item_id, number=number
                ),
            )
        )
        return markup
        ...
        markup.row(
            InlineKeyboardButton(
                text=f"0", callback_data="kk_data(item_id="")"
            )
    ...
    if int(summ) != 0:
        ...
        markup.row(
            InlineKeyboardButton(
                text="⬅️Ortga",
                callback_data=make_callback_data(
                    level=minus, category=category, subcategory=subcategory, item_id=item_id
                ),
            )
        )
        return markup
    ...
    if order:
        ...
        for i in order:
            ...
            markup.row(
                ...
                InlineKeyboardButton(
                    text=f"{produc_name['subcategory_name']}",
                    callback_data="name"
                ),
                InlineKeyboardButton(
                    text="+",
