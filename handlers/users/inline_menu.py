...
@dp.callback_query_handler(text="buyurtma") # menudan | qo'y go'sht mahsulotlariga "0" | buyurtmani bekor qilish
async def list_categories(message: Union[CallbackQuery, Message], **kwargs):
    print('251-qator')
    # Keyboardni chaqiramiz
...
async def show_item(callback: CallbackQuery, category, subcategory,item_id, number, summ, minus):
    print('157-qator')
...
async def delete_str(callback: CallbackQuery, category, subcategory, item_id, number, summ, minus):
    print('176-qator')
...
async def navigate(call: CallbackQuery, callback_data: dict):
    ...
    await current_level_function(
        call, count=count, item_id=item_id
    )
...
async def navigate(call: CallbackQuery, callback_data: dict):
    ...
    await current_level_function(
        call, product_id=product_id, count=count, minus=minus
    )
...
async def navigate(call: CallbackQuery, callback_data: dict):
    ...
    # Tanlangan funksiyani chaqiramiz va kerakli parametrlarni uzatamiz
    await current_level_function(
        call,category=category, subcategory=subcategory,item_id=item_id, number=number, summ=summ, minus=minus
    )
        #######
