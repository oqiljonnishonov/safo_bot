...
async def Menu(callback: CallbackQuery):
    ...
    await callback.message.answer("""<b>Maxsulotni olish usulini tanlang va manzilingizni yuboring 📍 </b>""",
                                  parse_mode="HTML",
...
async def location(message: Message):
    ...
    await message.answer("""<b>📍Manzilni to'g'ri yubordingizmi?</b>""", parse_mode="HTML",
                         reply_markup=knopka)
...
async def contact(message: Message, state: FSMContext):
    ...
    await message.answer("""<b>Olingan maxsulotlarga izoh qoldiring</b>""", parse_mode="HTML",
                         reply_markup=izoh)
...
async def izohlar(message: Message):
    ...
    bool = True
    await db.update_user_bool(user_id, bool)
    await message.answer("""<b>Olingan maxsulotlarga izoh qoldiring</b>""", parse_mode="HTML", reply_markup=izoh)
...
async def tekshirish(message: Message):
    ...
    await message.bot.send_message(text=msg,
                                   chat_id=telegram_id, reply_markup=tasdiq)
...
async def to_admin(message: Message):
    ...
    if order:
        ...
        await message.bot.send_message(text="Xaridingiz uchun rahmat! Sizning ma'lumotlaringiz tasdiqlandi",
                                       chat_id=telegram_id, reply_markup=ReplyKeyboardRemove())
        user = await  db.get_user(telegram_id)
        text = None
        ...
        for i in ADMINS:
            ...
            if user['latitude']:
                await message.bot.send_location(chat_id=i, latitude=user['latitude'], longitude=user['longitude'],
...
async def bekorqilish(call: Message):
    ...
    await call.answer("Buyurtma bekor qilindi", reply_markup=ReplyKeyboardRemove(), )
